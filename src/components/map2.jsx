...
function Map() {
  ...
  useEffect(() => {
    ...
    styleTag.textContent = customTooltipStyle;
  ...
  const getFontSize = () => {
    ...
    if (zoomLevel > 12) return "12px";
    return "12px";
  };

  const shouldDisplayTooltip = () => zoomLevel > 11;
        ...
        {lapaks.map((lapak) => (
          ...
          >
            {shouldDisplayTooltip() && (
              <Tooltip permanent direction="bottom" offset={[0, 15]} className="custom-tooltip" opacity={1} >
                <div
                  style={{
                    color: getColorForCategory(lapak.kategori_lapak),
                    fontWeight: "bold",
