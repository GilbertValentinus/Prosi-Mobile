...
function LapakList() {
  ...
  return (
    <div className="min-h-screen bg-[#161A32] text-white p-6">
      ...
      {error ? (
        <div className="text-red-500 p-4 rounded bg-red-900/20">
          {error}
        </div>
      ) : lapaks.length === 0 ? (
        ...
        <div className="max-h-[60vh] overflow-y-auto space-y-4">
          {lapaks.map((lapak) => (
            <div key={lapak.id} className="border-b border-gray-700 pb-4 flex justify-between items-center">
              <div>
                <Link to={`/detaillapak/${lapak.id}`}>
                  ...
                  <p className={`text-[13px] ${getStatusClassName(lapak.status)}`}>Status: {lapak.status}</p>
                </Link>
              </div>
              <div className="flex space-x-4">
                {lapak.status === 'terverifikasi' ? (
                  <Link to={`/editlapak/${lapak.id}`} className="text-blue-500 hover:text-blue-400 text-sm">
                    Edit
                  </Link>
                ) : lapak.status === 'terblokir' ? (
                  <button className="text-yellow-500 hover:text-yellow-400 text-sm" onClick={() => handleRequestUnblock(lapak.id)}>
                    Ajukan Lepas Blokir
                  </button>
                ) : null}
                {lapak.status !== 'terblokir' && (
                  <button className="text-red-500 hover:text-red-400 text-sm" onClick={() => handleDelete(lapak.id)}>
                    Hapus
                  </button>
                )}
              </div>
